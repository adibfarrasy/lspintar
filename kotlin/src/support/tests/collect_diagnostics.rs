...
#[test]
fn test_aliased_import_uses_alias_for_usage_check() {
    let source = r#"
import java.util.HashMap as HM
class Foo {
    val m: HM<String, Int> = HM()
}
"#;
    let diags = diagnostics_for(source);
    assert!(
        ...
        "aliased import used via alias should not be flagged as unused"
    );
}
...
#[test]
fn test_type_refs_captures_property_type() {
    let source = r#"
class Foo {
    val field: Bar = TODO()
}
...
#[test]
fn test_declared_types_captures_class_and_interface() {
    let source = r#"
class Alpha
interface Beta
"#;
...
fn test_class_decl_captures_name_and_parents() {
    let source = r#"
class Child : Parent(), Runnable, Serializable
"#;
...
fn missing_return_diags_for(source: &str) -> Vec<tower_lsp::lsp_types::Diagnostic> {
...
#[test]
fn test_unit_function_not_flagged() {
    let source = r#"
class Foo {
    fun doThing() {
        println("hi")
...
"#;
    assert!(missing_return_diags_for(source).is_empty(), "Unit function should not be flagged");
}

#[test]
fn test_explicit_unit_not_flagged() {
    let source = r#"
class Foo {
    fun doThing(): Unit {
        println("hi")
...
"#;
    assert!(
        missing_return_diags_for(source).is_empty(),
        "explicit Unit return type should not be flagged"
    );
}

#[test]
fn test_non_unit_with_return_not_flagged() {
    let source = r#"
class Foo {
    fun getName(): String {
        return "foo"
...
"#;
    assert!(
        missing_return_diags_for(source).is_empty(),
        "function with return should not be flagged"
    );
}

#[test]
fn test_non_unit_without_return_flagged() {
    let source = r#"
class Foo {
    fun getName(): String {
        println("oops")
...
"#;
    let diags = missing_return_diags_for(source);
    assert_eq!(diags.len(), 1, "expected one missing_return_statement diagnostic");
    assert_eq!(diags[0].severity, Some(DiagnosticSeverity::ERROR));
    assert!(diags[0].message.contains("getName"));
}

#[test]
fn test_expression_body_not_flagged() {
    let source = r#"
class Foo {
...
"#;
    assert!(
        missing_return_diags_for(source).is_empty(),
        "expression body functions always return and should not be flagged"
    );
}

#[test]
fn test_abstract_function_not_flagged() {
    let source = r#"
abstract class Base {
...
"#;
    assert!(
        missing_return_diags_for(source).is_empty(),
        "abstract function has no body and should not be flagged"
    );
}

#[test]
fn test_lambda_return_does_not_satisfy_outer_function() {
    let source = r#"
class Foo {
    fun compute(): String {
        val r = Runnable { return@Runnable }
    }
}
"#;
    let diags = missing_return_diags_for(source);
    assert_eq!(
        diags.len(),
        1,
        "return inside lambda should not satisfy outer function return requirement"
    );
}
...
#[test]
fn test_plain_as_cast_not_flagged() {
    let source = r#"
class Foo {
    fun test(obj: Any): String = obj as String
}
"#;
    assert!(
        unchecked_cast_diags_for(source).is_empty(),
        "cast to non-generic type should not be flagged"
    );
}

#[test]
fn test_generic_as_cast_flagged() {
    let source = r#"
class Foo {
    fun test(obj: Any): List<String> = obj as List<String>
}
"#;
...
#[test]
fn test_safe_cast_to_generic_flagged() {
    let source = r#"
class Foo {
    fun test(obj: Any): List<String>? = obj as? List<String>
}
"#;
    let diags = unchecked_cast_diags_for(source);
    assert_eq!(diags.len(), 1, "safe cast (as?) to generic type is also unchecked");
    assert_eq!(diags[0].severity, Some(DiagnosticSeverity::WARNING));
}
