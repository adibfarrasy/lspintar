...
use crate::core::{
    ...
    utils::{find_external_dependency_root, find_project_root, node_to_lsp_location, uri_to_path},
};
    ...
    if let Some(parent) = usage_node.parent() {
        if parent.kind() == "navigation_expression" {
            if let Some(enum_type_node) = parent.child(0) {
                if let Some(enum_type_name) =
                    super::project::resolve_nested_enum_type(source, &enum_type_node)
                {
...
fn search_external_definition_and_convert(
    ...
    let definition_node = {
        ...
        if source_info
            .zip_internal_path
            .as_ref()
            .map_or(false, |p| p.ends_with(".class"))
        {
