    ...
    for (query, _symbol_type) in queries {
        ...
        matches.for_each(|query_match| {
            ...
            if let Some(name) = symbol_name {
                ...
                if is_java_symbol_accessible(&name_node, &parsed_file.content) {
...
fn is_java_symbol_accessible(node: &Node, content: &str) -> bool {
    ...
    // Java default visibility rules:
    // - Classes, interfaces, enums: package-private by default, but indexed anyway
    // - Methods: package-private by default
    // - Fields: package-private by default
    match decl.kind() {
        "class_declaration"
