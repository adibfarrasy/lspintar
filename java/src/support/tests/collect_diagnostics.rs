...
use lsp_core::language_support::{LanguageSupport, ClassDeclarationData};
...
#[test]
fn test_wildcard_imports_not_flagged_as_duplicate() {
    let source = r#"
import java.util.*;
import java.util.*;
class Foo {}
"#;
    // Wildcard duplicate_import is still flagged
    let diags = diagnostics_for(source);
    let dup: Vec<_> = diags
        .iter()
        .filter(|d| {
...
"#;
    let decls = class_decls_for(source);
    // Interfaces should not be in the class_declarations result
...
#[test]
fn test_void_method_not_flagged() {
    let source = r#"
class Foo {
    void doThing() {
...
"#;
    assert!(missing_return_diags_for(source).is_empty(), "void method should not be flagged");
}

#[test]
fn test_non_void_with_return_not_flagged() {
    let source = r#"
class Foo {
    String getName() {
        return "foo";
...
"#;
    assert!(
        ...
        "method with return should not be flagged"
    );
}

#[test]
fn test_non_void_without_return_flagged() {
    let source = r#"
class Foo {
    String getName() {
...
#[test]
fn test_abstract_method_not_flagged() {
    let source = r#"
abstract class Base {
...
"#;
    assert!(
        missing_return_diags_for(source).is_empty(),
        "abstract method has no body and should not be flagged"
    );
}

#[test]
fn test_return_inside_if_counts() {
    let source = r#"
class Foo {
    String getName() {
        if (true) {
...
"#;
    assert!(
        missing_return_diags_for(source).is_empty(),
        "method with return inside if should not be flagged"
    );
}

#[test]
fn test_lambda_return_does_not_satisfy_outer_method() {
    let source = r#"
class Foo {
    String compute() {
        Runnable r = () -> { return; };
...
"#;
    ...
    assert_eq!(
        diags.len(),
        1,
        "return inside lambda should not satisfy outer method return requirement"
    );
}
...
#[test]
fn test_raw_type_cast_not_flagged() {
    let source = r#"
import java.util.List;
class Foo {
    void test(Object obj) {
...
"#;
    assert!(
        unchecked_cast_diags_for(source).is_empty(),
        "cast to raw type has no type arguments to erase"
    );
}
